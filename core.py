...
class Env(gym.Env):
    ...
    def monitor(self):
        ...
        if not hasattr(self, '_monitor'):
            ...
            if self.n is None:
                raise error.Error(
                    'You must call "configure()" before accesssing the monitor for {}'.format(self))
...
class Wrapper(Env, gym.Wrapper):
    ...
    def __init__(self, env=None):
        ...
        if env is not None and not env.metadata.get('runtime.vectorized'):
            if self.autovectorize:
                ...
                raise error.Error(
                    'This wrapper can only wrap vectorized envs (i.e. where env.metadata["runtime.vectorized"] = True), not {}. Set "self.autovectorize = True" to automatically add a Vectorize wrapper.'.format(env))
    ...
    def _configure(self, **kwargs):
        ...
        assert self.env.n is not None, "Did not set self.env.n: self.n={} self.env={} self={}".format(
            self.env.n, self.env, self)
        self.n = self.env.n
