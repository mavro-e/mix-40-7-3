...
class DualProxyServer(VNCProxyServer):
    ...
    def __init__(self, action_queue=None, error_buffer=None, enable_logging=True):
        ...
        super(DualProxyServer, self).__init__(
            action_queue, error_buffer, enable_logging)
    ...
    def recv_ClientInit(self, block):
        ...
        self.reward_proxy_thread = threading.Thread(
            target=lambda: self.reward_proxy.expect(pexpect.EOF))
        self.reward_proxy_thread.start()

        self._log_info(
            'Reward proxy server is up %s', self.reward_proxy.before)
    ...
    def close(self):
        ...
        if self.log_manager:
            os.system('/app/universe/bin/upload_directory.sh demonstrator_%(recorder_id)s %(directory)s %(bucket)s' %
                      dict(
                      recorder_id=self.factory.recorder_id, directory=self.logfile_dir,
