...
def check_with_output(command, *args, **kwargs):
    ...
    if proc.returncode != 0:
        raise Error(
            'Command {} returned non-zero exit status {}'.format(command, proc.returncode))
    return stdout
...
class VNCEnvDiscovery(object):
    ...
    def __init__(self):
        self.context = 'sci'
        self.namespace = 'gym'
        self.kubectl = ['kubectl', '--context',
                        self.context, '--namespace', self.namespace]

    def discover_batches(self):
        pods = check_with_output(
            self.kubectl + ['get', 'pods', '-o', 'json', '-l', 'type=universe'])
    ...
    def discover(self, batch, force_ready=False):
        pods = check_with_output(
            self.kubectl + ['get', 'pods', '-o', 'json', '-l', 'type=universe', '-l', 'batch={}'.format(batch)])
        pods = json.loads(pods)
        ...
        for pod in pods['items']:
            ...
            if node is None:
                if force_ready:
                    raise Error(
                        'Not all pods ready: {} is not scheduled on a node yet'.format(name))
                continue
