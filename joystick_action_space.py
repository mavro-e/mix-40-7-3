...
class JoystickActionSpace(gym.Space):
    ...
    def __init__(
        self, axis_x=False, axis_y=False, axis_z=False, axis_rx=False, axis_ry=False, axis_rz=False,
            slider_0=False, slider_1=False):
        self.event_space_map = OrderedDict()

        if axis_x:
            self.axis_x = box_axis()
            self.event_space_map[
                joystick_event.JoystickAxisXEvent] = self.axis_x
        if axis_y:
            self.axis_y = box_axis()
            self.event_space_map[
                joystick_event.JoystickAxisYEvent] = self.axis_y
        if axis_z:
            self.axis_z = box_axis()
            self.event_space_map[
                joystick_event.JoystickAxisZEvent] = self.axis_z
        if axis_rx:
            self.axis_rx = box_axis()
            self.event_space_map[
                joystick_event.JoystickAxisRxEvent] = self.axis_rx
        if axis_ry:
            self.axis_ry = box_axis()
            self.event_space_map[
                joystick_event.JoystickAxisRyEvent] = self.axis_ry
        if axis_rz:
            self.axis_rz = box_axis()
            self.event_space_map[
                joystick_event.JoystickAxisRzEvent] = self.axis_rz
        if slider_0:
            self.slider_0 = box_axis()
            self.event_space_map[
                joystick_event.JoystickSlider0Event] = self.slider_0
        if slider_1:
            self.slider_1 = box_axis()
            self.event_space_map[
                joystick_event.JoystickSlider1Event] = self.slider_1
