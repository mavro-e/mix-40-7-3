...
/**
 * This class retrieves the annotation map from application scope.  This map was placed there by the
 * JSFAnnotationProcessor
 * in the jsf subsystem.
 *
 * The class also reloads the map if needed.  The reason why the map must be reloaded is because the JSF Annotation
 * classes used as the map keys are always
...
public class AnnotationMap {
    ...
    private static final Map<String, Class<? extends Annotation>> stringToAnnoMap
    ...
    public static Map<Class<? extends Annotation>, Set<Class<?>>> get(final ServletContext servletContext) {
        Map<Class<? extends Annotation>, Set<Class<?>>> annotations
                = (Map<Class<? extends Annotation>, Set<Class<?>>>)servletContext.getAttribute(
    ...
    private static Map<Class<? extends Annotation>, Set<Class<?>>> convert(
            Map<Class<? extends Annotation>, Set<Class<?>>> annotations) {
        final Map<Class<? extends Annotation>, Set<Class<?>>> convertedAnnotatedClasses
        ...
        for (Map.Entry<Class<? extends Annotation>, Set<Class<?>>> entry : annotations.entrySet()) {
            ...
            if (knownAnnotation != null) {
                convertedAnnotatedClasses.put(knownAnnotation,
                                              annotated);  // put back in the map with the proper version of the class
            } else {
