...
public class ControlPointUtils {
    ...
    public static Runnable doWrap(Runnable runnable, ControlPoint controlPoint) {
        ...
        try {
            ...
            return runnable instanceof ManagedTask
                    ? new ControlledManagedRunnable(controlledRunnable, (ManagedTask)runnable)
                    : controlledRunnable;
        } catch (Exception e) { throw new RejectedExecutionException(e); }
    }
    ...
    public static <T> Callable<T> doWrap(Callable<T> callable, ControlPoint controlPoint) {
        ...
        try {
            ...
            return callable instanceof ManagedTask
                    ? new ControlledManagedCallable(controlledCallable, (ManagedTask)callable)
                    : controlledCallable;
        } catch (Exception e) { throw new RejectedExecutionException(e); }
    }
    ...
    public static Runnable doScheduledWrap(Runnable runnable, ControlPoint controlPoint) {
        if (controlPoint == null || runnable == null) {
            ...
            final ControlledScheduledRunnable controlledScheduledRunnable
                    = new ControlledScheduledRunnable(runnable, controlPoint);
            return runnable instanceof ManagedTask
                    ? new ControlledManagedRunnable(controlledScheduledRunnable, (ManagedTask)runnable)
    ...
    public static <T> Callable<T> doScheduledWrap(Callable<T> callable, ControlPoint controlPoint) {
        if (controlPoint == null || callable == null) {
            ...
            final ControlledScheduledCallable controlledScheduledCallable
                    = new ControlledScheduledCallable(callable, controlPoint);
            return callable instanceof ManagedTask
                    ? new ControlledManagedCallable(controlledScheduledCallable, (ManagedTask)callable)
    ...
    static class ControlledRunnable implements Runnable {
        ...
        public void run() {
            try {
                runnable.run();
            } finally { controlPoint.requestComplete(); }
        }
    }
    ...
    static class ControlledScheduledRunnable implements Runnable {
        ...
        public void run() {
            if (controlPoint == null) {
                ...
                } catch (Exception e) { EeLogger.ROOT_LOGGER.failedToRunTask(e); }
        }
    }
    ...
    static class ControlledManagedTask implements ManagedTask {
        ...
        ControlledManagedTask(ManagedTask managedTask) {
            ...
            this.managedTaskListenerWrapper
                    = managedTask.getManagedTaskListener() != null
                              ? new ControlledManagedTaskListener(managedTask.getManagedTaskListener())
                              : null;
        }
